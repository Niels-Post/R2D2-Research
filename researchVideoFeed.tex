...
\begin{enumerate}
	...
	\item Vraag 5 gaan we beantwoorden volgens toetsend onderzoek en met de hypothese zoals beschreven in het hoofdstuk ”Theorie en hypothese.” We nemen aan dat er niet voldoende geheugen is op Arduino om 1 frame op te slaan, dus verlagen we in stappen de resolutie. We splitsen het in opeenvolgende stappen van 5% op. We bufferen eerst 100% van de maximale resolutie, en kijken of het past op het geheugen. Dan 95% en kijken of dat past, etc. Uiteindelijk weten we dan dus hoeveel procent van één frame met de maximale resolutie gebufferd kan worden op de Arduino Due. Deze 5 % stappen zijn bewust want als de code iets zou uitbreiden moet er ook ruimte op het Ram zijn daarvoor. We willen dus niet op de 1% nauwkeurig het geheugen gaan vullen.
...
We moeten voor deze deelvraag de resolutie steeds in stappen van 5% verlagen. Daarbij kunnen we niet gewoon ${(640 \times 480) \times 0,95 = 29.184}$ doen. Want je moet de verhouding tussen de hoogte en breedte nog steeds hebben om een fatsoenlijk zichtbare foto te maken.
...
Dus in ons geval, om bijvoorbeeld 50% te berekenen, gebruiken we de volgende formule voor de breedte: ${1120 \times 0,5 \times 0,571}$. Dit geeft ons een waarde van 320.
...
In onze implementatie liep de Arduino Due na 75% nog steeds vast, omdat wij hierna nog een tweede matrix aanmaken om de RGB waarden op te slaan. Om twee arrays van die grootte te bufferen, heeft de Arduino Due dus niet genoeg geheugen. Als code geschreven wordt die in 1 keer de waardes verwerkt en dat dan buffert zodat er maar 1 matrix is (zoals bovenstaand omschreven), is het bufferen van 1 frame met een resolutie van 75% van 640 x 480 mogelijk. Het deel in de tabel wat met een enkele array van RGB waarden wel zou werken is met de oranje kleur gemarkeerd.

Pas bij een resolutie van 55% van het maximum van een frame doet onze implementatie met 2 arrays het pas echt. Hierbij werd de arduino geflashed, de data uitgelezen en in de eerste array gezet, de tweede array aangemaakt en gevuld met RGB waarden.
...
\begin{itemize}
\item \citet{i2cmemfault} 
\item \citet{bmp280adafruit}
\item \citet{computernerdgithub}
\item \citet{dpvtechyoutube}
\item \citet{makerpro}
\item \citet{hattori2008architecture}
\item \citet{i2cinfo}
\item \citet{indrekyoutube}
\item \citet{ov7670instructables}
\item \citet{lupyuenyoutube}
\item \citet{matanbrightarduino}
\item \citet{wiki:megapixel}
\item \citet{mt9m001arducam}
\item \citet{mt9m001utronics}
\item \citet{mrarduinoarduino}
\item \citet{ov7670voti}
\item \citet{ov7670implementationgithub}
\item \citet{ov7670opencircuit}
\item \citet{latexoverleaf}
\item \citet{ov7670interfaceegprojects}
\item \citet{i2crobotelectronics}
\item \citet{song2019development}
\item \citet{configregistersstackexchange}
\item \citet{surtrtechyoutube}
\item \citet{vela2018face}
\item \citet{connectov7670instructables}
